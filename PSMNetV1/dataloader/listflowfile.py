...
def dataloader(filepath):
    ...
    for dd in os.listdir(monkaa_path):
        for im in os.listdir(monkaa_path + '/' + dd + '/left/'):
            ...
            # if os.path.exists(monkaa_path + '/' + dd + '/right/' + im):
                all_right_img.append(monkaa_path + '/' + dd + '/right/' + im)
            # else:
            #     raise FileNotFoundError()
            # if os.path.exists(monkaa_disp + '/' + dd + '/left/' + im.split(".")[0] + '.pfm'):
        ...
        for j in subdir2:
            for k in subdir3:
                ...
                for im in imm_l:
                    ...
                    # if os.path.exists(driving_dir + i + '/' + j + '/' + k + '/right/' + im):
    ...
    for ss in subdir:
        for ff in os.listdir(flying_dir + ss):
            ...
            for im in imm_l:
                ...
                # if os.path.exists(flying_dir + ss + '/' + ff + '/right/' + im):
                    all_right_img.append(flying_dir + ss + '/' + ff + '/right/' + im)
                # else:
                #     raise FileNotFoundError()
                # if os.path.exists(flying_disp + '/TRAIN/' + ss + '/' + ff + '/left/' + im.split(".")[0] + '.pfm'):
    ...
    for ss in subdir:
        for ff in os.listdir(flying_dir + ss):
            ...
            for im in imm_l:
                ...
                # else:
                #     raise FileNotFoundError()
                # if os.path.exists(flying_disp + '/TEST/' + ss + '/' + ff + '/left/' + im.split(".")[0] + '.pfm'):
...
if __name__ == '__main__':
    filepath = 'F:\\3d_recons\\stereo_data\\SceneFlow\\'
    dataloader(filepath)
